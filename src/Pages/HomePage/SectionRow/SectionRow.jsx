...
function SectionRow({ header, apiGetData, styles, genres, pageNumber, pageSize }) {
  ...
  useEffect(() => {
    if (isInView && Object.keys(dataResult).length == 0) {
  ...
  const generateCardSection = useMemo(() => {

    return < div className={cx("container-card")} >
      {
        dataResult.bookResponses?.map((item) => (
  ...
  const generateSkeletonCard = useMemo(() => {
    const cards = []
    for (let i = 0; i < 24; i++) {
      cards.push(
